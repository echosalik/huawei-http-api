...
pub fn get_session_token() -> Result<models::response::SessionToken, Errors>{
  ...
  let url = format!("http://{}/api/webserver/SesTokInfo", dotenv::var("SMS_HOST")?);
...
pub fn login(username: String, password: String, token: String, session : String) -> Result<(models::response::SessionToken, models::response::Responses), Errors>{
  let client : reqwest::Client = reqwest::Client::new();
  ...
  let mut resp : reqwest::Response = client.post(url.as_str()).body(
    serde_xml_rs::to_string(&request)?
  ).headers(create_headers(token.clone(), session.clone(), "4".to_owned(), None)?).send()?;
  let text = resp.text().unwrap();
  let response = serde_xml_rs::from_str(text.as_str())?;
  return Ok((get_response_headers(&resp), response));
}

pub fn get_sms(token: String, session : String, unread : bool) -> Result<(models::response::SessionToken, models::response::Responses), Errors>{
  let client : reqwest::Client = reqwest::Client::new();
  ...
  let mut resp : reqwest::Response = client.post(url.as_str()).body(
    serde_xml_rs::to_string(&request)?
  ).headers(create_headers(token.clone(), session.clone(), "1".to_owned(), None)?).send()?;
  let text : String = resp.text()?;
  let response : models::response::Responses;
  let sms_empty_enum : Option<models::response::SMSGetEmpty> = match serde_xml_rs::from_str(text.as_str()) {
    Ok(data) => Some(data),
    Err(_e) => None,
  };
...
pub fn send_sms(token: String, session : String, number: Vec<String>, message: String) -> Result<(models::response::SessionToken, models::response::Responses), Errors>{
  let client : reqwest::Client = reqwest::Client::new();
...
pub fn read_sms(token: String, session : String, index: i32) -> Result<(models::response::SessionToken, models::response::Responses), Errors>{
  let client : reqwest::Client = reqwest::Client::new();
  let url = format!("http://{}/api/sms/send-sms", dotenv::var("SMS_HOST")?);
  let request = models::request::SMSRead {
    index: index,
  };
  let mut resp : reqwest::Response = client.post(url.as_str()).body(
    serde_xml_rs::to_string(&request)?
...
pub fn delete_sms(token: String, session : String, index: i32) -> Result<(models::response::SessionToken, models::response::Responses), Errors>{
  let client : reqwest::Client = reqwest::Client::new();
...
fn create_headers(token: String, session: String, dnt: String, encoding: Option<String>) -> Result<reqwest::header::HeaderMap, Errors> {
  let mut headers = reqwest::header::HeaderMap::new();
  headers.insert(
    reqwest::header::COOKIE, 
    reqwest::header::HeaderValue::from_bytes(session.as_bytes()).unwrap()
  );
  headers.insert(
    reqwest::header::HeaderName::from_bytes("__RequestVerificationToken".as_bytes()).unwrap(),
    reqwest::header::HeaderValue::from_bytes(token.as_bytes()).unwrap()
  );
  headers.insert(
    reqwest::header::HeaderName::from_bytes("DNT".as_bytes()).unwrap(),
    reqwest::header::HeaderValue::from_bytes(dnt.as_bytes()).unwrap()
  );
  if encoding.is_some() {
    headers.insert(
      reqwest::header::CONTENT_TYPE,
      reqwest::header::HeaderValue::from_bytes(encoding.unwrap().as_bytes()).unwrap()
    );
  }
  return Ok(headers);
}
